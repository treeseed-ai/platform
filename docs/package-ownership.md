...
`treeseed-ai/platform` is the canonical installer and integration workspace. It bundles Admin, API, Agent, AI, CLI, Core, Reviewer, SDK, TreeDX, UI, and the Engineering and Research templates. It never checks out, provisions, deploys, or reconciles Market or Market API. Every project owns one derived TreeDX virtual knowledge repository; it is a logical knowledge resource, not a Git checkout or submodule.
...
Project architecture is project-first. A project may own a primary software repository, or it may be content-only with no Git repository. In both cases the control plane derives exactly one TreeDX virtual knowledge repository. TreeDX owns its file, blob, search, graph, context, workspace, and artifact workflows; R2 may serve immutable runtime publications. Virtual knowledge repositories have no GitHub branch, PR, Actions, or package-release policy and are retired with their owning project. See [Project Architecture Migration](./project-architecture-migration.md).

First-party software packages may still declare local source-content paths and R2 publication settings in `treeseed.package.yaml`, but those paths do not create a second repository identity. Package release gates remain independent: missing `docs/` sites report `site_not_prepared`, while package CI and publishing continue to follow the manifest's existing `verify`, `releaseGate`, and artifact settings.
...
| `@treeseed/sdk` | Portable contracts and remote interfaces | Canonical control-plane operation descriptors, wire schemas, standards/compatibility contracts, generated remote control-plane clients, OAuth protocol clients, and the project-oriented `TreeSeedTreeDxClient` facade. It owns no control-plane persistence, application services, direct TreeDX transport, TreeDX credentials, or TreeDX OpenAPI copy. |
| `@treeseed/api` | Public Admin and platform control-plane API, AGPL-3.0-only with a commercial alternative | Identity, teams, projects, governance, capacity, workdays, assignments, audit, realtime, PostgreSQL state, OAuth, REST/OpenAPI, MCP, and application services. It is the sole TreeSeed TreeDX proxy: it resolves bindings and scope, mints short-lived delegations, calls the official TreeDX client, and returns normalized receipts without copying TreeDX content. Commerce does not belong here. |
...
| `@treeseed/cli` | Human/operator command surface | The sole `trsd` executable: command-tree parsing, generated help/completion, server/session custody, confirmation interaction, local secret handling, and rendering over SDK operation IDs. It contains no API application logic, direct TreeDX client, raw remote paths, repository integration internals, or Agent runtime passthrough. |
...
| `@treeseed/agent` | Capacity-provider and agent runtime | Provider manager/runner runtime, AgentKernel execution, lifecycle telemetry, execution-provider adapters, provider-local capacity enforcement, and assignment-scoped tools. Trusted runtime code injects a project-fixed TreeSeed API facade and cancels it with lease authority; executors receive no TreeDX endpoint, direct client, delegation credential, topology, or unrestricted binding. |
...
| `packages/treedx` | Independent generic repository data/index/query service | Owns its service implementation, authoritative OpenAPI, generated TypeScript/Python/Rust/Elixir clients, storage, Git/repository graph/indexing, federation, artifacts, and images. It has no TreeSeed product semantics and no dependency on `@treeseed/sdk`. |
...
- Content creation and update requests are validated against SDK-owned portable schemas before the API authorizes a TreeDX proxy operation. The API invokes the official TreeDX client; SDK consumers invoke only the TreeSeed semantic facade. Core runs raw frontmatter through the portable model registry before its Astro schemas add runtime defaults. Diagnostics preserve exact path, model, and field data for CLI, API, Admin, and agent-chat repair loops without creating a second TreeDX backend or credential path.
...
- Singleton reconciliation owns only the declared gateway, descriptor, verification, and manifest overlay inside `treeseed-ai/market-api`. Market application files are repository-owned: the reconciler bootstraps the extension entrypoint once, preserves all paths outside its manifest, and blocks managed-file drift instead of replacing the application tree.
