...
For split-content projects, local TreeDX reconciliation observes the authoritative GitHub staging head, fetches that exact ref into TreeDX custody, re-observes GitHub to reject stale plans, and verifies graph/search parity at the resolved commit. It must never populate a split-content TreeDX repository from the primary software checkout. Repeated reconciliation is `noop` only while the TreeDX ref and live GitHub ref remain identical.

Content publication has a model-validation admission boundary after exact UTF-8 source observation and before runtime-manifest compilation or R2 mutation. The SDK-owned portable Zod registry validates every recognized content collection, normalizes canonical aliases, and rejects the publication with path, model, field, code, and message diagnostics. Supporting collections without a Treeseed content model may pass through, but adding a new modeled collection requires registering its schema before it is publication-authoritative. Pull-request `--validate-only` workflows use this same reconciler, so package-local and integrated publication do not maintain separate validation paths.

Private split-content repositories use the same graph without exposing reusable provider credentials to the SDK or TreeDX desired state. The local adapter asks the platform-runner-authenticated Admin API to verify the exact live repository and head, reconcile the project repository binding, and prepare a project-, node-, refspec-, operation-, and commit-scoped delivery. The API resolves `TREESEED_GITHUB_TOKEN` server-side for first-party `treeseed-ai/*` repositories and returns only an opaque, sealed, one-use delivery ID. TreeDX consumes that ID through its credential broker, and plans, state, journals, logs, and verification reports contain no token. An unexpired ready delivery is retryable, an interrupted grant is recovered, and a consumed or expired delivery creates a fresh bounded attempt. Convergence still requires a fresh live head read and a final `noop` plan.

Initial repository resources are provisioned in two mutually exclusive authority domains. The Platform seed may create thirteen public primary repositories, their thirteen public content repositories, and the unpaired fixtures support repository. The singleton Market seed may create Market, Market content, the private Market API, and its private content repository. After creation, their teams' live project/repository inventories—not either seed—own current membership and bindings. Platform compilation must reject Market service, repository, DNS, or deployment identities before provider planning. The built-in `treeseed` Market profile is immutable outside process-scoped local fixtures and contributes no infrastructure resources.
...
9. Local build mutations fail closed before crossing the protected host-disk reserve. Docker image reconciliation computes build headroom from the prepared context, performs one bounded BuildKit-cache recovery attempt under pressure, verifies capacity again, and never treats cache pruning as permission to delete images, volumes, workday evidence, databases, or TreeDX state.
...
GitHub adapters currently cover repository metadata, bootstrap/branches, branch rules, environments, secrets, variables, workflow dispatch and observation, package release workflows, and image workflows. The target provider-neutral work-item and change-request adapters add synchronized Issues, pull requests, reviews, checks, templates, and merge read-back without transferring lifecycle authority from the API; see [GitHub Issues and Pull Request Integration](./github-work-integration.md). First-party repositories currently resolve the single central token, with repository-scoped overrides limited to imported third-party projects; the target authentication contract prefers a least-privilege GitHub App while retaining explicit token support.
...
Local Agent capacity uses one content-addressed runtime image tagged for the provider manager and provider runner roles, while hosted publication retains separate role images. The local seed bootstrap observes each generated provider manifest against its owning seed and source-manifest digest, applies that exact owner seed when stale, and persists convergence only after the regenerated runtime manifest passes authoritative read-back.
...
The persistent local Platform service has two deliberate interruption modes. `trsd platform pause` stops the current user service while preserving enablement; `trsd platform stop` disables and stops it so normal shutdown does not cause a later automatic restart. Platform status reports inventory-backed repository identity, branch/upstream, local head, dirty state, ahead/behind counts, active runtime state, and an actionable repair command. App-scoped dev restart selects only that app's dependency closure; it does not implicitly restart capacity providers or unrelated TreeDX content reconciliation.
