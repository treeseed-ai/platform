...
- Next action: complete exact-head hosted verification and independent review for the SDK, CLI, API, and Agent definition-compatibility/PR-contract heads; integrate SDK to staging first, then re-read and integrate the three exact SDK consumers. After source read-back, reconcile the same definitions through local TreeDX and rerun SDK planning from a new immutable generation. Keep npm publication blocked.
