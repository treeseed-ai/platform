...
- State: `in_progress`
- Active repository: `treeseed-ai/sdk`
- Completed bootstrap repositories: `treeseed-ai/cli` and `treeseed-ai/platform`
- Next action: complete independent review and exact-head hosted checks for SDK PR #1 at `7bdb291d…`; merge/read back the narrow CI repair only if green. Keep npm publication blocked while reconciling staging's breaking export-map drift from published `0.12.62`.
...
| SDK baseline repair candidate | `7bdb291dd8989359e9c94e39d22e3acd8b88ca3c` | Draft PR [#1](https://github.com/treeseed-ai/sdk/pull/1), exact base `f843c3cb…`; local proposal `6795bdc7…`, accepted decision `85bd44c4…`; focused and full isolated acceptance green; review/hosted checks pending |
...
7. **SDK canary — in review** — fresh assignment checkout at exact `f843c3cb…`; local proposal `6795bdc7-2cd2-40a9-aaee-c53cb1407fb7` and accepted decision `85bd44c4-e6a5-45cd-9d44-96cce898d4b3`; draft PR [#1](https://github.com/treeseed-ai/sdk/pull/1) at `7bdb291dd8989359e9c94e39d22e3acd8b88ca3c`. The diff changes only the malformed type-query and standalone local-provider test fixture; `c6db3626…` was not adopted. Clean install added 637 packages. Focused tests passed 2 files/9 tests; isolated release verification passed 391 files/1,532 tests, lifecycle 6 files/60 tests, and packed-install smoke. The packed candidate SHA-256 is `7466e0f9ca9eb0dbd7eccf1b2c8ca5e81779a4fa381aa8e5a928e057d3f1f325`. Independent review, hosted exact-head verification, merge/read-back, settlement, and cleanup remain pending.
8. **Patch proof — blocked for publication, active for diagnosis** — consumer heads were snapshotted before the SDK change: API `b18379fe…`, Agent `6e1a734…`, CLI `23a3869d…`, Core `32a5c1e…`, Admin `1565686…`, UI `d622525…`; none was modified. Comparing the exact staged candidate with published `0.12.62` found 81 baseline versus 90 candidate export entries: 15 additions, 6 removals, and 5 candidate export declarations missing from the tarball. Therefore `f843c3cb…` cannot truthfully publish as contract-unchanged `0.12.63`. The narrow CI repair may merge to staging after review, but main merge/npm publication remain fail-closed pending a separate compatibility decision and export/package repair.
