...
- Next action: plan the governed local TreeDX/project-agent-class deployment from the four exact final staging refs, without publishing content externally; then issue the required human credential-rotation checkpoint before scheduling a fresh immutable SDK simulation. Keep SDK main and npm publication blocked.
...
- Next executable action: plan governed local TreeDX/project-agent-class deployment from the exact SDK, CLI, API, and Agent staging refs; preserve local-only custody and issue the human provider-credential rotation checkpoint before applying deployment or scheduling a new immutable SDK run
...
- Residual gates: live API/CLI/Agent project agent classes remain absent; governed local TreeDX deployment, class/grant reconciliation, baseline/clean-repeat/interruption runs, main read-back, and provider credential rotation remain open. Main/npm and production publication remain fail-closed.
