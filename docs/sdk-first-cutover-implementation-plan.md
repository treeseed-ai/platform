...
- Next action: run exact-head checks on the common staging-content correction PRs SDK #5, CLI #7, API #2, and Agent #3; obtain the API contributor affirmation under `H-005`; independently review, merge, and prove validation-only Content runs on staging. Keep SDK main and npm publication blocked.
- Pending human checkpoints: `H-001` Ubuntu host discovery is non-blocking for guest-side work. `H-004` is verified. `H-005` is waiting for the API contributor affirmation.
...
- Next executable action: correct the shared staging Content contract through SDK #5, CLI #7, API #2, and Agent #3, require exact-head checks and independent review, merge/read back all four, and prove post-merge staging Content validation succeeds without R2 credentials; then settle/clean the superseded run, rotate the provider credential, deploy the reviewed definitions through local TreeDX, and schedule a new immutable SDK run before creating any writable project checkout
