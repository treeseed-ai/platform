...
- Next action: wait for CLI staging run `32207567279`, Agent staging run `32207568045`, and corrected API PR head `f952c17e…` checks/re-review. If API is green, close its PR metadata, merge/read back staging, and require its staging Verify/artifact. Keep SDK main and npm publication blocked.
...
- Agent-definition audit: `docs/agent-definition-capacity-compatibility-audit.md` is the bounded SDK/CLI/API/Agent audit. Its original scan found 56 read-only-branch/content-write conflicts across 28 of 32 definitions. The remediated exact worktrees report 32 passing definitions and zero findings. SDK PR [#4](https://github.com/treeseed-ai/sdk/pull/4) adds an internal compatibility evaluator and package-owned regression fixtures without changing the existing public validator contract. CLI PR [#6](https://github.com/treeseed-ai/cli/pull/6), API PR [#1](https://github.com/treeseed-ai/api/pull/1), and Agent PR [#2](https://github.com/treeseed-ai/agent/pull/2) still pin an obsolete intermediate SDK and must adopt the accepted SDK staging artifact plus its internal verifier in their own corrected heads. All four carry the common PR template; the corrected SDK validator is the source contract the consumers must adopt after SDK staging read-back. API alone carries its repository-policy contributor grant. These are candidate branches, not accepted staging evidence yet.
...
- Exact candidate heads: SDK staging `92a66135fbda5575bee058b0ab25339c6c7d6398`; CLI reviewed `2e9610a60cfddc7a7bea679487530667b10a8407`, staging `cafe7efbd29db3d58dbfbd841002b69b4e166572`; API `f952c17e103d888974b03cd58504542c4c946241`; Agent reviewed `b6f07113536d8420f0ac6c0e1c445dc38947062c`, staging `a444f1cca0cdd61e7854e219d1db243d08531129`
...
- Static acceptance: the original audit reports 32 definitions and zero compatibility diagnostics. Corrected SDK tests pass 34 agent-capacity/workflow files and 170 tests, plus `build:dist` and a negative public-export read-back. Earlier CLI, API, and Agent heads each passed a clean install and their eight-definition test against obsolete SDK `2de71415…`; those consumer results are now diagnostic only and must be repeated against the accepted SDK staging identity with explicit internal compatibility evaluation. CLI's direct-dependency npm override remains required to prevent mixed Git SDK instances.
...
- PR submission contract: `.github/PULL_REQUEST_TEMPLATE.md` collects outcome; work item, proposal/decision, assignment/checkpoint, actor, accountable human, agent/provider, exact base/head; contributor mode; plan/status; changes/commits; verification; risk/rollback; completion summary; and readiness checks. Corrected SDK validation compares recorded full SHAs to the provider base/head, requires exactly one contributor mode, and requires all readiness boxes only when the PR leaves draft. It executes only base-owned workflow code, never checks out PR code, and pins its action. CLI/API/Agent still carry the earlier nonempty-only validator blob `1699c3c30db31a2a6ef1b37d1f3c6d2a00da8283` and must adopt the corrected SDK contract after SDK staging read-back. API retains its exact human contributor grant, which passed under `H-004`. The active GitHub CLI OAuth session lacks `workflow`; the already-authorized account SSH identity publishes reviewed workflow commits without expanding OAuth scope.
...
- Consumer adoption: CLI `2e9610a60cfddc7a7bea679487530667b10a8407`, API `65495a70f08086671c6dccf55a675559a530409d`, and Agent `b6f07113536d8420f0ac6c0e1c445dc38947062c` now declare and lock SDK staging `92a66135fbda5575bee058b0ab25339c6c7d6398`, explicitly run the internal compatibility evaluator after public structural validation, and carry the byte-identical corrected PR validator. API hydration classifies accepted `92a66135…` as run `32205531009` and failed intermediate `2de71415…` as immediate `failed_fast`. Clean local installs passed: API 629 packages, CLI 1,202, Agent 620. Package-owned compatibility tests pass 1/1 in each repository; Agent required its declared fixture submodule at exact `940d1de…`. Fresh hosted checks are pending.
- Consumer review/integration: independent review found no findings in CLI `2e9610a6…` or Agent `b6f07113…`; all PR checks passed and push/PR artifacts were byte-identical at CLI SHA-256 `420c7c11e9430dfd4a9c0a5460bd2fd5ce0383aca96895e5d48d46570e335aa3` and Agent `3e5756ec99ecae0210a502e3019702dd66c8c29f9ff2eb29957319b4c4fd86af`. CLI PR #6 merged/read back on staging at `cafe7efbd29db3d58dbfbd841002b69b4e166572`; Agent PR #2 merged/read back at `a444f1cca0cdd61e7854e219d1db243d08531129`. Their staging runs `32207567279` and `32207568045` are pending. API `65495a70…` passed both Verify runs, Content, and affirmation with byte-identical artifact SHA-256 `41d9f01985857fc0d74335c0c7169295c34ac7539387e53d7196c3e90e4b54a1`, but review found a duplicate trailing Verification section in the PR template. Corrected API head `f952c17e103d888974b03cd58504542c4c946241` consolidates the checkbox into the primary section; fresh checks/re-review are pending.
