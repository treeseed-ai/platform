...
- Next action: wait for final SDK, CLI, and API post-merge package Verify runs; compare their exact staging artifacts where emitted, then mark the common definition cutover verified and proceed to settlement/local TreeDX deployment. Keep SDK main and npm publication blocked.
