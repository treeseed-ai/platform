...
- Next action: repair API's impossible SDK artifact hydration by moving its declared exact SDK dependency from failed producer commit `2de71415…` to verified candidate `429c4473…`, regenerate the bounded lock, and fail fast when a pinned producer has already failed; rerun exact-head API verification, then complete independent review for all four candidates. Integrate SDK to staging first and re-read consumers before their integration. Keep npm publication blocked.
- Pending human checkpoints: `H-001` Ubuntu host discovery is non-blocking for guest-side work. `H-004` is verified.
...
- Agent-definition audit: `docs/agent-definition-capacity-compatibility-audit.md` is the bounded SDK/CLI/API/Agent audit. Its original scan found 56 read-only-branch/content-write conflicts across 28 of 32 definitions. The remediated exact worktrees now report 32 passing definitions and zero findings. SDK PR [#4](https://github.com/treeseed-ai/sdk/pull/4) adds the portable compatibility evaluator and package-wide regression fixtures. CLI PR [#6](https://github.com/treeseed-ai/cli/pull/6), API PR [#1](https://github.com/treeseed-ai/api/pull/1), and Agent PR [#2](https://github.com/treeseed-ai/agent/pull/2) adopt its exact head and run package-owned definition checks. The four heads also add the same durable default PR template and base-trusted `pull_request_target` body contract; API alone adds its existing license-policy grant and reserves that affirmation for a human. These are candidate branches, not accepted staging evidence yet.
...
- Exact candidate heads: SDK `429c4473877fc278022b0abfac317028e121d451`; CLI `1b961c9d05ab224ded2383ceb543f5a1eb858330`; API `063f323aafd5eed03f873edc34c71fd0a36115f6`; Agent `bbe857622344f69ebd9a538ffd1c95d35b565eac`
...
- Static acceptance: 32 packaged definitions pass with zero compatibility diagnostics. SDK agent-capacity tests pass 30 files/147 tests; focused schema/compatibility tests pass 9/9 and the corrected capacity-contract fixture passes in a 14/14 focused run; SDK build and architecture checks pass. CLI, API, and Agent each passed a clean install and its package-owned eight-definition check. The consumers pin evaluator source commit `2de71415477d7e19c332aa8062fe0f587d709651`; later SDK commit `2cb803ab…` changes only the SDK contract test fixture, not shipped evaluator/package source. CLI additionally requires `overrides: { "@treeseed/sdk": "$@treeseed/sdk" }` so its Agent/Core/UI closure cannot install multiple incompatible Git SDK instances; `npm ci --dry-run` now passes for all three consumers.
- Hosted correction: the first SDK head failed architecture because the new evaluator created an eleventh direct validation file, and a feature-branch Content push attempted external R2 publication. Commit `2de71415…` moves the evaluator under functional ownership and limits content publication pushes to `main`/`staging`; PR validation remains local and credential-free. The next exact-head run exposed one pre-existing read-only question-answerer fixture that relied on implicit mutation presets; SDK commit `2cb803ab…` declares the four content mutation tools denied and passes the focused contract. Agent commit `0f619e0b…` likewise moves its regression test into a functional subdomain after the first head exceeded the ten-file test-root limit. The current heads add the PR submission contract without changing shipped package source. Fresh hosted checks are pending.
- PR submission contract: `.github/PULL_REQUEST_TEMPLATE.md` now collects outcome; work item, proposal/decision, assignment/checkpoint, actor, accountable human, agent/provider, exact base/head; contributor mode; plan/status; changes/commits; verification; risk/rollback; completion summary; and readiness checks. Drafts may retain pending completion items, while `pull_request_target` validation requires the durable identity fields and exactly one contributor mode and requires all readiness boxes before a PR is marked ready. The validator executes only base-owned workflow code and never checks out the untrusted PR. API retains its exact contributor-license statement, provides a direct `CONTRIBUTING.md` link, and explicitly prohibits an agent from checking the human grant. All four current PR bodies were migrated to this format and pass direct evaluation of the exact workflow script at their recorded heads. Remote read-back reports the same workflow blob `1699c3c30db31a2a6ef1b37d1f3c6d2a00da8283` in all four branches. The active GitHub CLI OAuth session advertised `admin:public_key, gist, read:org, repo` but not `workflow`, so HTTPS correctly rejected workflow mutation; the already-authorized account SSH identity published the exact commits without expanding OAuth scope. The human API grant passed under `H-004`.
- API hosted failure: Verify runs [`32202582218`](https://github.com/treeseed-ai/api/actions/runs/32202582218) and [`32202586194`](https://github.com/treeseed-ai/api/actions/runs/32202586194) timed out after twenty minutes in `Hydrate exact SDK artifact`; no API dependency install or test ran. API declares SDK commit `2de71415477d7e19c332aa8062fe0f587d709651`, whose two SDK Verify runs failed and emitted no artifact. Current SDK candidate `429c4473877fc278022b0abfac317028e121d451` passed push run [`32202582149`](https://github.com/treeseed-ai/sdk/actions/runs/32202582149) and PR run [`32202586609`](https://github.com/treeseed-ai/sdk/actions/runs/32202586609); the push run exposes non-expired artifact `sdk-429c4473877fc278022b0abfac317028e121d451`. The repair must align API's declared dependency, lock, and hydrated artifact and replace blind polling with a bounded fail-fast producer diagnostic.
...
| Definition/PR-contract candidates | SDK `429c4473877fc278022b0abfac317028e121d451`; CLI `1b961c9d05ab224ded2383ceb543f5a1eb858330`; API `063f323aafd5eed03f873edc34c71fd0a36115f6`; Agent `bbe857622344f69ebd9a538ffd1c95d35b565eac` | Draft PRs target staging; exact-head hosted verification/review and staging read-back are pending. Consumers pin the shipped evaluator at SDK `2de71415…`; later SDK commits affect its contract fixture and repository submission metadata, not the shipped evaluator. |
