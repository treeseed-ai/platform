...
- PR submission contract: `.github/PULL_REQUEST_TEMPLATE.md` now collects outcome; work item, proposal/decision, assignment/checkpoint, actor, accountable human, agent/provider, exact base/head; contributor mode; plan/status; changes/commits; verification; risk/rollback; completion summary; and readiness checks. Drafts may retain pending completion items, while `pull_request_target` validation requires the durable identity fields and exactly one contributor mode and requires all readiness boxes before a PR is marked ready. The validator executes only base-owned workflow code and never checks out the untrusted PR. API retains its exact contributor-license statement, provides a direct `CONTRIBUTING.md` link, and explicitly prohibits an agent from checking the human grant. All four current PR bodies were migrated to this format; the API grant remains intentionally unchecked pending `H-004`.
