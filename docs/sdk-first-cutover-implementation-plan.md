...
- Next action: integrate the repository-neutral SDK Agent Lab correction in draft PR [#6](https://github.com/treeseed-ai/sdk/pull/6), hydrate the local operations runner from the accepted SDK staging artifact, and replay failed SDK simulation operation `e683ba77-936c-4785-822e-89dc572bfb5c` from immutable TreeDX commit `37853fc374ecd710131f79cf9070edd915c95874`. Keep SDK main and npm publication blocked.
- Pending human checkpoints: `H-001` Ubuntu host discovery is non-blocking for guest-side work. `H-006`, `H-004`, and `H-005` are verified.
...
- Repository-neutral source repair: SDK draft PR [#6](https://github.com/treeseed-ai/sdk/pull/6), exact base `cf9ae76edc181fe80cacf4fe1aa9c582e08a22d5` and head `71a85ff5c25f2773f3f3e3055542db0a8e669bd4`, replaces the Market-name exception with a one-authorized-repository invariant and preserves the selected repository slug in evidence. Focused Agent Lab tests pass 31/31, `build:dist` passes, and `git diff --check` passes. Hosted checks, independent review, governed checkpoint adoption, staging read-back, runner hydration, replay, clean repeat, interruption/resume, settlement, and residue proof remain open.
- Next executable action: complete SDK PR #6 through the common review/check/staging path, hydrate the local operations runner from the accepted SDK staging artifact, and replay the immutable SDK simulation. In parallel, prepare the Agent rotation-offer preservation regression as a separate common-lifecycle source change.
