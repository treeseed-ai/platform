...
for (const project of Array.isArray(payload.projects) ? payload.projects : []) {
	...
	for (const repository of Array.isArray(project.repositories) ? project.repositories : []) {
		...
		if (!['primary', 'fixture'].includes(role) || !owner || !name || ['market', 'market-api', 'platform'].includes(name) || name.endsWith('-content')) continue;
...
console.log(JSON.stringify({ ok: true, teamId, count: inventory.length, inventoryDigest: `sha256:${digest}`, receiptStatus: receipt.status }));
